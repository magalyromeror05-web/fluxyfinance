...
import {
  ...
  Landmark, Unplug, Loader2, Clock, WifiOff, Search, ArrowLeft, ShieldCheck,
} from "lucide-react";
...
export default function Connections() {
  ...
  const handleConfirmConnect = useCallback(async () => {
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.functions.invoke("pluggy-auth");
      ...
      const { data: conn, error: connErr } = await supabase
        ...
        .insert({
          ...
          country: "BR",
          status: "connected",
          external_connection_id: authData?.connectToken || "pending",
        })
        .select("id")
        .single();
      ...
      toast.success("Banco conectado! Sincronização em andamento...");
      queryClient.invalidateQueries({ queryKey: ["connections"] });
      resetModal();
    } catch (err: any) {
